...
function parseHTML(html, baseURL, policy, { mode }) {
    ...
    // ★ 追加: <object data> / <embed src> → object-src と frame-src 両方に反映
    doc.querySelectorAll('object[data]').forEach(el => {
        addOrigin(policy['object-src'], el.data, baseURL);
        addOrigin(policy['frame-src'], el.data, baseURL);
    });
    doc.querySelectorAll('embed[src]').forEach(el => {
        addOrigin(policy['object-src'], el.src, baseURL);
    ...
    if (mode !== 'nonce') {
        for (const raw of extractRawInlineScripts(html)) {
            for (const v of variantsForHashing(raw)) policy['script-src'].add(`'sha256-${sha256(v)}'`);
        }
        for (const s of doc.querySelectorAll('script:not([src])')) {
            const code = s.textContent ?? '';
            for (const v of variantsForHashing(code)) policy['script-src'].add(`'sha256-${sha256(v)}'`);
        }
    }

    // ② on* 属性（raw/trim 両方）＋ 'unsafe-hashes'
    if (mode !== 'nonce') {
        for (const el of doc.querySelectorAll('*')) {
            for (const attr of el.attributes) {
                if (/^on/i.test(attr.name)) {
                    ...
                    if (raw) {
                        policy['script-src'].add(`'sha256-${sha256(raw)}'`);
                        const t = raw.trim(); if (t && t !== raw) policy['script-src'].add(`'sha256-${sha256(t)}'`);
                    }
                }
            }
        }
    ...
    const addDevOrigins = (text) => {
        ...
        for (const m of text.matchAll(DEV_RE)) {
            try {
                ...
                policy['script-src'].add(origin); policy['connect-src'].add(origin);
            } catch { }
        }
    };
    addDevOrigins(html);
    doc.querySelectorAll('script:not([src])').forEach(s => addDevOrigins(s.textContent || ''));
