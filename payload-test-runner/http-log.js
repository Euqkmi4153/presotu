...
async function serveLocalHTML(res, url, filePath) {
    try {
        ...
        if (reportOnlyHeader) {
            headers['content-security-policy-report-only'] = appendUpgradeInsecureRequests(reportOnlyHeader);
        }
    ...
    const upstream = https.request({
        ...
        proxyRes.on('end', async () => {
            try {
                ...
                if (reportOnlyHeader) {
                    out['content-security-policy-report-only'] = appendUpgradeInsecureRequests(reportOnlyHeader);
                }
