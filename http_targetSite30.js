...
function sanitizeReqHeaders(h) {
    ...
    Object.keys(o).forEach((k) => {
        if (typeof o[k] === 'undefined') delete o[k];
    });
    return o;
}
function sanitizeResHeaders(h) {
    ...
    delete o['trailer'];
    delete o['upgrade'];
    if (typeof o['transfer-encoding'] === 'string' && o['transfer-encoding'] !== '')
...
function injectHelpers(html) {
    ...
    if (/<head[^>]*>/i.test(html) && !/<base\b[^>]*>/i.test(html)) {
        html = html.replace(
            /<head([^>]*)>/i,
            `<head$1><base href="/_localfs/">`
        );
    }
    // securitypolicyviolation ダミー（常に用意・既存は上書きしない）
    const helper =
        `<script>window.securitypolicyviolation||` +
        `(window.securitypolicyviolation=function(p){` +
        `try{console.log('[CSP-violation]',p)}catch(_){}});</script>`;
...
async function handleProxyRequest(req, res, targetUrl) {
    ...
    if (u.protocol !== 'https:') {
        res.writeHead(400);
        res.end('Only https scheme is allowed');
        return;
    }
    ...
    const perform = (urlObj, lookupFn, redirects = 5) =>
        new Promise((resolve) => {
            const opts = {
                ...
                if (
                    ...
                    try {
                        ...
                        if (next.protocol !== 'https:') {
                            upRes.resume();
                            return resolve({ error: new Error('non-HTTPS redirect') });
                        }
                        upRes.resume();
                        return resolve(perform(next, lookupFn, redirects - 1));
                    } catch (e) {
                        upRes.resume();
...
async function serveLocalHTML(res, url, filePath) {
    try {
        ...
        const { enforceHeader, reportOnlyHeader } = await generateCSP(url, html, {
            mode: MODE,
        ...
        const headers = {
            ...
            'content-security-policy': appendUpgradeInsecureRequests(
                enforceHeader || ''
            ),
        };
        if (reportOnlyHeader)
            headers['content-security-policy-report-only'] =
                appendUpgradeInsecureRequests(reportOnlyHeader);
...
http
    .createServer((req, res) => {
        ...
        const raw =
            req.url && req.url.startsWith('http')
                ? req.url
                : `${scheme}://${req.headers.host}${req.url}`;
        let targetUrl;
        try {
        ...
        if (targetUrl.pathname === '/csp-report') {
            ...
            req.on('end', () => {
                try {
                    console.log('[CSP-REPORT]', Buffer.concat(body).toString('utf8'));
                } catch { }
                res.writeHead(204);
                res.end();
            });
        ...
        // 動的挿入用の /script.mjs をローカルスタブで返す（targetSite27）
        if (targetUrl.pathname === '/script.mjs') {
            const js =
                "/* local script.mjs stub */\n" +
                "console.log('[LOCAL script.mjs] loaded');";
            const buf = Buffer.from(js, 'utf8');
            res.writeHead(200, {
                'content-type': 'application/javascript; charset=utf-8',
                'cache-control': 'no-store',
        ...
        // targetSite29 用の /style.css スタブ
        if (targetUrl.pathname === '/style.css') {
            const css =
        ...
        if (normalizedHostname === 'not-example.com') {
            ...
            if (/^\/flash(?:\/|$)/.test(targetUrl.pathname)) {
                const svg =
                    ...
                    `font-family="sans-serif" font-size="18" fill="#fff">fake SWF content</text></svg>`;
            ...
            if (/^\/styles\/.+\.css$/i.test(targetUrl.pathname)) {
                const css =
                    ...
                    'h1::after{content:" (from not-example.com CSS)"}';
                const buf = Buffer.from(css, 'utf8');
                res.writeHead(200, {
                    'content-type': 'text/css; charset=utf-8',
                    'content-length': String(buf.byteLength),
            ...
            // default image/svg stub
            const svg =
                `<svg xmlns="http://www.w3.org/2000/svg" width="200" height="120">` +
                `<rect width="100%" height="100%" fill="#4caf50"/>` +
                `<text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" ` +
                `font-family="sans-serif" font-size="16" fill="#fff">stub image</text></svg>`;
            const buf = Buffer.from(svg, 'utf8');
            res.writeHead(200, {
                'content-type': 'image/svg+xml; charset=utf-8',
                'content-length': String(buf.byteLength),
        ...
        if (targetUrl.pathname.startsWith('/_localfs/')) {
            ...
            const resolved = path.resolve(abs),
                rootRes = path.resolve(LOCAL_ROOT);
            ...
            fs.readFile(resolved, (err, buf) => {
                if (err) {
                    // ★ ここを 404 にすることで <img src="1" onerror=...> 等のテストで onerror が発火する
                    res.writeHead(404, {
            ...
            (proxyRes) => {
                ...
                proxyRes.on('end', async () => {
                    try {
                        ...
                        const decoded = await maybeDecodeBody(
                            raw,
                        ...
                        out['content-security-policy'] =
                            appendUpgradeInsecureRequests(enforceHeader || '');
                        if (reportOnlyHeader)
                            out['content-security-policy-report-only'] =
                                appendUpgradeInsecureRequests(reportOnlyHeader);
    ...
    .listen(8080, '0.0.0.0', () => {
        ...
        console.log(
            '[INIT] ready _local_test1..30 ; upstream HTTPS fixed; CSP by csp-generator.mjs'
        );
    });
