...
http
    .createServer((req, res) => {
        ...
        if (normalizedHostname === 'not-example.com') {
            ...
            // video stub — 再生は目的ではなく、リクエスト発生が目的
        ...
        // ★ /script.mjs をローカルファイルまたはスタブJSとして返す（targetSite27 用）
        if (targetUrl.pathname === '/script.mjs') {
            const localScript = path.join(LOCAL_ROOT, 'script.mjs');
            fs.readFile(localScript, (err, buf) => {
                if (err) {
                    // ファイルが無い場合はスタブJSを返す
                    const js =
                        `/* local script.mjs stub */\n` +
                        `console.log('[LOCAL script.mjs stub loaded]');`;
                    const b = Buffer.from(js, 'utf8');
                    res.writeHead(200, {
                        'content-type': 'application/javascript; charset=utf-8',
                        'cache-control': 'no-store',
        ...
        if (targetUrl.pathname.startsWith('/_localfs/')) {
            ...
            fs.readFile(resolved, (err, buf) => {
                if (err) {
                    // 404 にすることで <img src="1" onerror=...> 等のテストで onerror が発火
                    res.writeHead(404, {
    ...
    .listen(8080, '0.0.0.0', () => {
        ...
        console.log(
            '[INIT] ready _local_test1..27 ; upstream HTTPS fixed; CSP by csp-generator.mjs',
        );
    });
